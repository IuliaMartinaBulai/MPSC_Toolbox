...
if length(varargin)>0
    props.currentCase = varargin{1};
    if length(varargin)>1
        props.currentSubcase = varargin{2};
    else
        props.currentSubcase = ['analysis_', date];
    end
else
    props.currentCase = ['analysis2_', date];
    props.currentSubcase = ['analysis2_', date];
end
...
if ~isunix
    mkdir([casePath, '\', props.currentCase, '\', props.currentSubcase]);
    addpath([casePath, '\', props.currentCase, '\', props.currentSubcase]);
else
    mkdir([casePath, '/', props.currentCase, '/', props.currentSubcase]);
    addpath([casePath, '/', props.currentCase, '/', props.currentSubcase]);
end
...
props.subCasePath = [casePath, '/', props.currentCase, '/', props.currentSubcase];
